...
export default function Login() {
  ...
  const handleSubmit = (e) => {
    ...
    setError('');
    setTimeout(() => {
      // If email contains 'tutor', log in as tutor
      const role = email.includes('tutor') ? 'tutor' : 'student';
      const dest = login(email, password, role);
      navigate(dest === 'tutor' ? '/tutor-dashboard' : '/dashboard');
    }, 700);
  };
        ...
        <div className="auth-card fade-in">
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-3">
              ...
              <input className="form-control-custom" type="email" placeholder="john.doe@university.edu"
            ...
            {error && <div style={{ background: '#f8d7da', color: '#721c24', borderRadius: 8, padding: '0.6rem 1rem', fontSize: '0.88rem', marginBottom: '1rem' }}>{error}</div>}
